...
struct ParticleEmitter {
    ...
    std::vector<Quad> update(double curr_time, Renderer* renderer, BufferID global_data_buffer) {
        particles.erase(
        ...
        if (curr_time - last_updated > emission_delay + curr_emmision_delay_var) {
            ...
            const float curr_emission_angle_var = 0.0;
            const float particle_angle = emission_angle + curr_emission_angle_var;
            const glm::vec2 particle_dir = glm::vec2(cos(particle_angle), sin(particle_angle));
        ...
        for (const Particle& particle : particles) {
            quads.push_back(particle.get_quad(curr_time, renderer, global_data_buffer, texture));
        }
        return quads;
    }
